    ...
    def jgl_cpu_can_win(self, jgl_winning_combos):
        ...
        # Condition suggested by CoPilot
            if combination.count(self.jgl_cpu_symbol) == 2 and combination.count("___") == 1 \
                and self.jgl_is_winning_combination(combination):
                    jgl_empty_box_index = combination.index("___")
                    jgl_copy_board = self.jgl_game.jgl_game_board.copy()
                    jgl_copy_board[jgl_empty_box_index] = f"_{self.jgl_user_symbol}_"
                    if all(jgl_copy_board[i] == f"_{self.jgl_user_symbol}_" \
                        for i in jgl_winning_combos):
                            return True
                    return False
                
                    
    def jgl_cpu_can_block(self, jgl_winning_combos):
        ...
        for combination in jgl_winning_combos:
            if combination.count(self.jgl_user_symbol) == 2 and combination.count("___") == 1:
                    ...
                    if self.jgl_is_winning_combination(jgl_copy_board):
                            return True
        return False
